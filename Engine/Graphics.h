...
public:
	...
	void DrawRect_Border(Rect_<T , D> rect, Color c , E effect , RectI clip = GetScreenRect())
	{
		...
		for (int x = new_rect.left; x < new_rect.right(); x++)
		{
			effect(c, *this, x, new_rect.top() - 1);
		}
	}
	...
	void BlendPixel(int x, int y, Color c , int alpha)
	{
		...
		float old_color_alpha = 1.0f - tFactor;

		Color sColor = Color( float(pSysBuffer[index].GetR()) * old_color_alpha ,
							  float(pSysBuffer[index].GetG()) * old_color_alpha ,
							  float(pSysBuffer[index].GetB()) * old_color_alpha  );

		int r = (float(c.GetR() * tFactor) + sColor.GetR()), 
			b = (float(c.GetB() * tFactor) + sColor.GetB()),
			g = (float(c.GetG() * tFactor) + sColor.GetG());
	...
	template <typename T>
	void DrawLine(Vec2_<T> p1, Vec2_<T> p2, Color c, int thickness)
	{
		float pp_slope = p1.GetPPSlope(p2);

		//A will be(0.0f , 0.0f)
		Vec2D B(1.0f, pp_slope);
		Vec2D C(-1.0f, pp_slope);

		Vec2D AB = B / B.GetLenght();
		Vec2D 
	}
	template <typename T , typename D> 
	static bool WithinWorld(const Rect_<T , D>& rect) 
	{
		return rect.right() >= WorldArea.left && WorldArea.right() > rect.left &&
			rect.top() >= WorldArea.bottom && WorldArea.top() > rect.bottom;
	}
private:
...
public:
	...
	static constexpr int WorldWidth  = 2 * ScreenWidth;
	static constexpr int WorldHeight = 2 * ScreenHeight;
